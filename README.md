...
> 上記リンクの **Assets** から各OSのインストーラを取得できます。過去版は [リリース一覧](https://github.com/katusaburou/Rainbreak/releases) から。変更履歴は [CHANGELOG.md](CHANGELOG.md)。
...
> **開発状況（2026年6月時点）**: 正式版 **v1.0.0 を公開済み**です。macOS の実機検証（全画面共存・クリックスルー切替）は継続中 → [実装状況 / ロードマップ](#実装状況--ロードマップ)。
...
### (B) 集中中の穏やかな常時バー
時間を隠すのではなく「見せる、ただし穏やかに」。隅に**数字なしの連続バー**を常駐させ、ゴールが近づく感覚（goal-gradient）で集中を後押しします。バーは**ドラッグで好きな位置へ動かせ、縁をつかんで縦・横・斜めにサイズ調整**もできます（位置・サイズは記憶）。クリックしてもフォーカスを奪わないので、タイピングを中断させません。
...
4つのフェーズ（セット数を設定した場合は終端の「セット終了」を加えた5つ）を Rust 側の状態機械（`crates/core`・早送りの単体テストで遷移を固定）が駆動します。**時間の真実は常に Rust 側**にあり、WebView は描画と演出に専念します。
...
stateDiagram-v2
    ...
    state "雨上がり Clearing（3秒）\n雨フェードアウト→退避\n最終セットは10秒: 虹と雫・鳥の余韻" as Clearing
    state "セット終了 Finished\nタイマー停止 / HUDは満了表示" as Finished

    Work --> Incoming: 作業残り30秒
    Incoming --> Shower: 作業時間 満了
    Shower --> Clearing: 休憩時間 満了
    Clearing --> Work: フェード完了（ループ）
    Clearing --> Finished: 設定セット数を消化
    Finished --> Work: Skip（新しいセッション）
...
| **雨上がり・最終セット**（10秒） | 全画面→hide | 非表示のまま | ON→OFF | **ON** | 強→消滅→**虹** | フェードアウト→**雫と遠くの鳥** | Esc で切り上げ可（作業はすぐ再開できる） |
| **セット終了**（セット数設定時のみ） | hide | 表示（満了のまま） | OFF | — | なし | OFF | トレイの Skip で次のセッション |
...
flowchart TB
    ...
    subgraph Front["SvelteKit / WebView — 描画と演出"]
        direction TB
        RAIN["雨レンダリング<br/>raindrop-fx（WebGL2）<br/>劣化時は静的ベール"]
        HUDV["HUDバー（数字なし）"]
...
| 雨描画 | **raindrop-fx**（WebGL2） | 実画面のキャプチャを屈折元に使う「モードB」（降雨中 2.5 秒間隔で追従、キャプチャ不可時は静止画のモードA）。粒の合体・蛇行滑落・トレイルはライブラリが担い、雨脚 0..1 をパラメータへ写像。WebGL2 なし／reduced-motion／初期化失敗は雨なし＋CSS の静的ベールに統一（フォールバック描画は持たない）。 |
...
| パフォーマンス | 作業中（雨なし）・非表示・被覆時は描画を完全停止して省電力（raindrop-fx は内部 rAF 駆動）。 |
| アクセシビリティ | `prefers-reduced-motion` を**自動で尊重**（演出を簡略化／無効化 → [雨が表示されない場合](#トラブルシューティング)）。Skip／Esc を常備し、ユーザーを閉じ込めない。 |
...
OS の「動きを減らす」系のアクセシビリティ設定が有効だと、雨やどりは `prefers-reduced-motion` を尊重して**雨アニメーションを描かず、静的な暗いベールだけ**を表示します（仕様）。雨を表示したい場合は、次の OS 設定を**オフ**にしてから**アプリを再起動**してください（設定は起動時に一度だけ判定します）。

**macOS — 「視差効果を減らす」をオフにする**
- **macOS Ventura (13) 以降**: アップルメニュー → **システム設定** → **アクセシビリティ** → **ディスプレイ** → 「**視差効果を減らす**」をオフ。
- **macOS Monterey (12) 以前**: アップルメニュー → **システム環境設定** → **アクセシビリティ** → 左リストの**ディスプレイ** → 「ディスプレイ」タブ → 「**視差効果を減らす**」のチェックを外す。
- 設定の現在値はターミナルでも確認できます（`1` なら有効＝雨が出ない状態）:
  ```sh
  defaults read com.apple.universalaccess reduceMotion
  ```

**Windows — アニメーション効果をオンにする**
- **Windows 11**: **設定** → **アクセシビリティ** → **視覚効果** → 「**アニメーション効果**」をオン。
- **Windows 10**: **設定** → **簡単操作** → **ディスプレイ** → 「**Windows にアニメーションを表示する**」をオン。

> 上記がオフ（＝動きを減らさない設定）なのに雨が出ない場合は、WebGL2 が利用できない環境の可能性があります（その場合も同じ静的ベール表示になります）。
...
設定は「道具を管理させる」負債と捉え、**美的パラメータ（背景・雨量・演出強度）は作者が決め打ち**し、設定から外しています。残すのは次の4点のみで、値は永続化します。

- **作業時間 / 休憩時間**（デフォルト **20分 / 5分**）── 機能的可変項目。
- **セット数**（デフォルト **無制限**）── 設定した回数の作業サイクルを終えると、雨上がりのあとにタイマーが停止する（セット終了）。最終セットの雨上がりは少し長く（10秒）、雨が引いたあとに虹が架かり、軒先の雫と遠くの鳥の声が余韻として鳴る。トレイの Skip で次のセッションを開始。
- **音量 / ミュート** ── 好みではなく基本操作。
...
**2026年6月時点**: 正式版 **v1.0.0 を公開済み**です。コアの状態機械は早送りの単体テスト（22件）で遷移を固定し、CI とリリースワークフローも稼働中。残るは macOS の実機検証ゲートです。
...
- [x] 雨描画（raindrop-fx。屈折元は実画面キャプチャ＝モードB、不可時は自前背景1枚＝モードA。WebGL2 なし／reduced-motion 時は雨なし＋静的ベール）
- [x] 隅の HUD バー（数字なし・ドラッグで移動／縁で縦横斜めにリサイズ可＆位置サイズを記憶・フォーカスは奪わない・終盤だけ暖色へ寄る goal-gradient）
...
- [x] 設定UIと永続化（サイクル長・セット数・音量/ミュート・自動起動。OS 設定ディレクトリの JSON）
- [x] `prefers-reduced-motion` の自動尊重（雨アニメーションを止め、静的ベール表示へ）
